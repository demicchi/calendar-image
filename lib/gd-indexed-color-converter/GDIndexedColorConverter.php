...
namespace StudioDemmys\lib\GDIndexedColorConverter;
...
/**
 * Modified by demicchi
 * Modification also be licensed under The MIT License
 * @author demicchi
...
class GDIndexedColorConverter
{
     ...
     * @param bool $binary When true, return a binary array of indices of the color palette. When false, an image resource.
     * @return \GdImage|array The image resource in indexed color mode or the binary array of [y][x]
     */
    public function convertToIndexedColor ($im, $palette, $dither = 0.75, $binary = false)
    {
        $newPalette = array();
        foreach($palette as $color_index => $paletteColor) {
            $newPalette[] = array(
                'rgb' => $paletteColor,
        ...
        $newImage = $this->floydSteinbergDither($im, $width, $height, $newPalette, $dither, $binary);
     ...
     * @param bool $binary When true, return a binary array of indices of the color palette. When false, an image resource.
     * @return \GdImage|array The pixels after applying Floyd–Steinberg dithering or the binary array of [y][x]
     */
    private function floydSteinbergDither($im, $width, $height, &$palette, $amount, $binary = false)
    {
        ...
        for ($i = 0; $i < $height; $i++) {
            ...
            for ($j = 0; $j < $width; $j++) {
                ...
                $closestColor_array = $this->getClosestColor(array('rgb' => $color), $palette, 'rgb');
                $closestColor = $closestColor_array['rgb'];
                $binary_array[$i][$j] = $closestColor_array['color_index'];
        ...
        return $binary ? $binary_array : $newImage;
    }
