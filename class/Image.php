...
class Image
{
    ...
    public static function illuminateBox(\GdImage $image, int $x, int $y, int $width, int $height,
                                         array|int|Color|null $background_color = null,
                                         array|int|Color|null $line_color = null,
                                         string $shape = "rectangle") :void
    {
        if (!is_null($background_color)) {
            $background_color_int = self::getColorInt($image, $background_color);
            if ($background_color_int !== false) {
                match (strtolower($shape)) {
                    "circle" => imagefilledellipse($image, ($x + $x + $width - 1) / 2, ($y + $y + $height - 1) / 2,
                        $width, $height, $background_color_int),
                    default => imagefilledrectangle($image, $x, $y, $x + $width - 1, $y + $height - 1,
                        $background_color_int),
                };
            }
        }
        if (!is_null($line_color)) {
            $line_color_int = self::getColorInt($image, $line_color);
            if ($line_color_int !== false) {
                match (strtolower($shape)) {
                    "circle" => imageellipse($image, ($x + $x + $width - 1) / 2, ($y + $y + $height - 1) / 2,
                        $width, $height, $line_color_int),
                    default => imagerectangle($image, $x, $y, $x + $width - 1, $y + $height - 1,
                        $line_color_int),
                };
            }
        }
